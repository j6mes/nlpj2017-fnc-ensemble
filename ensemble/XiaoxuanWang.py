...
class XiaoxuanWang(Classifier):
    def __init__(self,dataset,train):
        ...
        if not os.path.isfile("features/tfidf.pickle"):
            self.tfidfs.update(self.precompute_tf_idfs(dataset,dataset,train))
    ...
    def precompute_tf_idfs(self,dataset,newdataset,train):
        headlines = []
        bodies = []
        for stance in train:
        ...
        headlines = []
        bodies = []
        for stance in newdataset.stances:
            headlines.append(stance['Headline'])
            bodies.append(newdataset.articles[stance['Body ID']])
        gen_set = []
        ...
        sim_scores = dict()
        for x, stance in tqdm(enumerate(newdataset.stances)):
    ...
    def prepare_final(self,dataset,finaldataset,train):
        ...
        self.tfidfs.update(self.precompute_tf_idfs(dataset,finaldataset,train))
